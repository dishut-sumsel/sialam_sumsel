...
<body>
    <div>
        <div id="map"
            @if (app('request')->input('d')) style="height: 300px; width: 400px;" @else style="height: 500px;" @endif>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" title="Tutup" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body" id="modal-container" name="modal-container" style="text-align: center">
                    <h5 class="icraf-orange">Daftar Istilah Perhutanan Sosial (PS)</h5>
                    <table id="tbl-legendps" class="table table-striped table-bordered">
                        ...
                        <tr>
                            <td>HKm</td>
                            <td>Hutan Kemasyarakatan</td>
                        </tr>
                        <tr>
                            <td>HTR</td>
                            <td>Hutan Tanaman Rakyat</td>
                        </tr>
                        <tr>
                            <td>KK</td>
                            <td>Kemitraan Kehutanan</td>
                        </tr>
                        <tr>
                            <td colspan="2">Fungsi Kawasan Hutan</td>
                        </tr>
                        <tr>
                            <td>HL</td>
                            <td>Hutan Lindung</td>
...
<script>
    setTimeout(() => {
            ...
            //         var Azure_Maps = L.tileLayer(
            //   'https://atlas.microsoft.com/map/tile?subscription-key={{ env('AZURE_MAPS_KEY') }}&api-version=2.1&tilesetId=microsoft.imagery&zoom={z}&x={x}&y={y}',
            //   { maxZoom: 22 }
            // );
            ...
            @if (app('request')->input('i'))
                ...
                region = function() {
                    ...
                    $.ajax({
                        url: APP_URL +
                            '/shpfile/load2/{{ $map->id }}/{{ app('request')->input('i') }}',
                        type: 'GET',
                        dataType: 'json',
                        async: false,
                                ...
                                if (lstFieldsRegional.includes(key)) {
                                    switch (key) {
                                        ...
                                        case 'nmdesa':
                                            ...
                                            break;
                                        case 'f_kws':
                                            key2 = 'Fungsi Kawasan Kehutanan';
                                            break;
                                        case 'feg_kghltr':
                                            key2 = 'Fungsi Kawasan Gambut';
                                            break;
                                        case 'skema_ps':
                                            key2 = 'Status PS';
                                            break;
                                    ...
                                    if (feature.properties[key] == 'Tidak direkomendasi PS' ||
                                        feature
                                        .properties[key] == '-')
                                        value = 'Sudah ada izin/sedang dalam pengajuan izin';
                                    else value = feature.properties[key];
                                    table += '<tr><td>' + key2 + '</td><td>' + value +
                                        '</td></tr>';
                                }
                                if (key == 'skema_ps' && (feature.properties[key] != '-' &&
                                        feature
                                        .properties[key] != 'Tidak direkomendasi PS')) {
                                    table +=
                                        '<tr><td colspan="2" style="text-align: right"><a class="text-white btn btn-primary btn-sm" href="#" onclick="openLandAccessApp(\'' +
                                        btoa(feature.properties['idd']) +
                                        '\')">Pilih dan Proses</a></td></tr>';
                                }
                            }
                            table += '</table>';
            ...
            @elseif (app('request')->input('d'))
                ...
                region = function() {
                    ...
                    $.ajax({
                        url: APP_URL +
                            '/shpfile/load3/{{ $map->id }}/{{ app('request')->input('d') }}',
                        type: 'GET',
                        dataType: 'json',
                        async: false,
                        success: function(data) {
                    ...
                    onEachFeature: function(feature, layer) {
                        ...
                        for (var key in feature.properties) {
                            if (lstFieldsRegional.includes(key)) {
                                ...
                                table += '<tr><td>' + key2 + '</td><td>' + value + '</td></tr>';
                            }
                        }
                        table += '</table>';
                        layer.bindPopup(table);
            ...
            @else
                ...
                region = function() {
                    var data;
                    $.ajax({
                        url: APP_URL + '/shpfile/load/{{ $map->id }}',
                        type: 'GET',
                        dataType: 'json',
                        async: false,
                        success: function(data) {
            ...
            L.control.layers(baseMaps, otherLayers, {
                ...
                const customControl = L.Control.extend({
                    ...
                    addLegend: function(text) {
                        ...
                        legend.onAdd = function() {
                            ...
                            div.setAttribute("style",
                                "background-color: lightgray; padding: 0 5px;")
                    ...
                    addMarker: function({
                        ...
                        return L.marker([latitude, longitude], {
                            ...
                            '<br><a href="#" onclick="processLayer2()">Proses</a>');
                    },
